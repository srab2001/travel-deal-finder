...
class FlightSearcher {
  ...
  _liveProvider() {
    return Boolean(this.apiKey && this.provider === 'kiwi' && this.fetchImpl);
  }

  async _reserveRateSlot() {
    const now = this.now();
    const earliest = Math.max(now, this.nextSlotAt);
    this.nextSlotAt = earliest + this.rateLimitMs;
  ...
  async scrapeGoogleFlights(departure, destination, outDate, returnDate) {
    ...
    // Rate limit only applies when actually hitting the provider; mock mode
    // is pure CPU and shouldn't pay a 2s gap per call.
    if (this._liveProvider()) {
      await this._reserveRateSlot();
    }
  ...
  async searchAllCombinations(config, { onProgress, year, concurrency } = {}) {
    const results = [];
    const errors = [];
    ...
    // Mock mode: no rate limit, no network — run the lot in parallel.
    // Real mode: cap parallelism so we don't slam the provider.
    const cap = concurrency ?? (this._liveProvider() ? DEFAULT_CONCURRENCY : combos.length || 1);
    ...
    const worker = async () => {
      while (true) {
        ...
        completed++;
        if (typeof onProgress === 'function') onProgress(completed, combos.length, c);
      }
    };

    const workers = Array.from({ length: Math.min(cap, combos.length || 1) }, () => worker());
...
module.exports = { FlightSearcher, runSearch, DEFAULT_CONCURRENCY };
